...
class ACMCalendar(baseslide.BaseSlide):
    ...
    def fetch_and_update(self):
        xml = xml.dom.minidom.parse(urllib2.urlopen(MBTA_39_URI))
        stops = xml.getElementsByTagName('predictions')
        # there's no foreach and it kills me to map without using the retval...
        for stop in stops:
